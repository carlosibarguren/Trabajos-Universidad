...
def validar_entradas(entry_prob_compra, entry_prob_repara, entry_prob_retira,
    ...
    try:
        ...
        prob_refrigerio = validar_float(entry_prob_refrigerio, "Probabilidad de refrigerio")
        paso_refrigerio = validar_float(entry_paso_refrigerio, "Paso Refrigerio")
        a_refrigerio = validar_float(entry_a_refrigerio, "Constante A")
        tiempo_simular = validar_float(entry_tiempo_simular, "Tiempo a Simular")
        valor_a = validar_float(entry_a, "Mostrar Desde")
        valor_b = validar_int(entry_b, "Cantidad Filas")

        # Si algún campo no es válido, retornar False
        if None in [prob_compra, prob_repara, prob_retira, cliente_desde, cliente_hasta,
                    venta_desde, venta_hasta, reparar_desde, reparar_hasta, prob_refrigerio, 
                    tiempo_simular, valor_a, valor_b]:
            return False
        ...
        # Validar probabilidad de refrigerio
        if not (0 <= prob_refrigerio <= 1):
            raise ValueError("La probabilidad de refrigerio debe estar entre 0 y 1.")

        # Validar tiempo a simular
        if tiempo_simular < 0:
            raise ValueError("El tiempo a simular no puede ser menor a 0.")
        
        if paso_refrigerio <= 0:
            raise ValueError("El Paso de Euler debe ser mayor a 0")
        if paso_refrigerio > tiempo_simular:
            raise ValueError("El Paso de Euler deber ser menor al tiempo de simulación")
